...
Функция ЗапускПриложения()
	...
	ПарсерКоманд.ДобавитьИменованныйПараметр("-cleanProcessed");
	...
	НеЧиститьФайлыКОбработке = ПараметрыЗапуска.Получить("-cleanProcessed") = "false";
	Если Не НеЧиститьФайлыКОбработке Тогда
		//УдалитьФайлы(ПараметрыЗапуска["-filesForProcess"]); //Очистка папки файлов к обработке
	КонецЕсли;
