...
Функция ЗапускПриложения()
	...
	Лог.Информация("Начинаем обработку");
	...
	ПарсерКоманд.ДобавитьИменованныйПараметр("-edtModule");
	ПарсерКоманд.ДобавитьИменованныйПараметр("-ib");
	...
	Лог.Информация("Количество изменений " + ЖурналИзменений.Количество());
	...
	ПараметрыОбработки.ПутьКБазе = ПараметрыЗапуска.Получить("-ib");
	ПараметрыОбработки.ВерсияЕдт = ПараметрыЗапуска.Получить("-edtModule");
	
	//ПараметрыЗапуска.Свойство("-ib", ПараметрыОбработки.ПутьКБазе);
	//ПараметрыОбработки.ПутьКБазе = ПараметрыЗапуска["-ib"];
	...
	Лог.Информация("Количество файлов к обработке " + ФайлыКОбработке.Количество());
	
	ВыполнитьОбработкуФайлов(ФайлыКОбработке, ПараметрыОбработки);
	
	Если ПолучитьЖурналИзменений().Количество() > 0 Тогда
		
		Лог.Информация("Коммит в репозиторий");

		Комментарий = "Разбор отчетов и обработок";
		АвторДляГит = "Администратор <a.borodavko@21vek.by>";
		...
		ВнешРепозитории = РепозиторийGit.ПолучитьСписокВнешнихРепозиториев();

		Если ВнешРепозитории.Количество() > 0 И ЗначениеЗаполнено(ВнешРепозитории[0].Имя) Тогда
			Лог.Информация("Отправка в удаленный репозиторий");
			РепозиторийGit.Отправить();
		КонецЕсли;
	КонецЕсли;
	
	ЧиститьТемп = Истина;
	Если ЧиститьТемп Тогда
		УдалитьФайлы(ТекущийКаталогИсходныхФайлов); //Очистка темпа
	КонецЕсли;
...
Функция ПолучитьСтандартныеПараметрыОбработки() Экспорт
	...
	ПараметрыОбработки.Вставить("ПутьКБазе", "");
	ПараметрыОбработки.Вставить("ВерсияЕдт", "");
...
Функция ПолучитьЖурналИзменений()
	...
	ПараметрыКомандыGit.Добавить("status --short --no-renames -u");
	...
	Если Не ПустаяСтрока(РезультатВывода) Тогда
...
Процедура ДобавитьФайлыВРепозиторий(ПапкаСФайлами, ПапкаРепозитория)
	...
	ПапкаОбработок = ОбъединитьПути(ПапкаРепозитория, "Файлы обработок");
	
	ФайлПапкаОбработок = Новый Файл(ПапкаОбработок);
	
	Если НЕ ФайлПапкаОбработок.Существует() Тогда
		СоздатьКаталог(ПапкаОбработок);
	КонецЕсли;
	
	Для каждого ФайлОтчетаОбработки Из СписокФайлов Цикл
		
		НовыйПуть = ОбъединитьПути(ПапкаОбработок, ФайлОтчетаОбработки.Имя);
		...
		Попытка
			...
			ФайлОбработки = Новый Файл(НовыйПуть);
			Если ФайлОбработки.Существует() Тогда
				УдалитьФайлы(НовыйПуть);
			КонецЕсли;
			
			ПереместитьФайл(ФайлОтчетаОбработки.ПолноеИмя, НовыйПуть);
		Исключение
