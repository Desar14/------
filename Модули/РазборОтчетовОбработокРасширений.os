...
Функция ОбработатьФайл(АнализируемыйФайл, КаталогИсходныхФайлов, ДополнительныеПараметры) Экспорт
	...
	Если ТипФайлаПоддерживается(АнализируемыйФайл) Тогда
		...
		Если СтрСравнить(АнализируемыйФайл.Расширение, ".cfe") <> 0 Тогда
			...
			Если ДополнительныеПараметры.Свойство("ПутьКБазе") Тогда
				ПутьКБазе = ДополнительныеПараметры.ПутьКБазе;
			Иначе
				ПутьКБазе = "";
			КонецЕсли;
			
			Если ДополнительныеПараметры.Свойство("ВерсияЕдт") Тогда
				ВерсияЕдт = ДополнительныеПараметры.ВерсияЕдт;
			Иначе
				ВерсияЕдт = "edt";
			КонецЕсли;
			
			РаспаковатьОтчетОбработку(АнализируемыйФайл, КаталогВыгрузки, ИспользуемаяВерсияПлатформы, ПутьКБазе);
			
			СконвертироватьВПроектЕДТ(КаталогВыгрузки,
				ПутьРабочееПространство,
				ПутьПроект, ВерсияЕдт);
...
Процедура РаспаковатьОтчетОбработку(Знач Файл, Знач КаталогВыгрузки, ИспользуемаяВерсияПлатформы, ПутьКБазе = "")
	
	Лог.Отладка("Распаковка файла внешнего отчета обработки %1", Файл.ПолноеИмя);
	
	Конфигуратор = ПодготовитьКонфигуратор(ИспользуемаяВерсияПлатформы, ПутьКБазе);
	...
	Лог.Отладка("Вывод 1С:Предприятия - " + Вывод);
	
КонецПроцедуры

Функция ПодготовитьКонфигуратор(ИспользуемаяВерсияПлатформы, ПутьКБазе)
	...
	Если ЗначениеЗаполнено(ПутьКБазе) Тогда
		Конфигуратор.УстановитьКонтекст(ПутьКБазе, "", "");
	КонецЕсли;
...
Процедура СконвертироватьВПроектЕДТ(КаталогФайловОбработки, ВременнаяПапкаВоркспейса, КаталогИсходниковВФорматеЕДТ, ВерсияЕдт)
	...
	Команда.УстановитьСтрокуЗапуска("ring " + ВерсияЕдт + " workspace import");
